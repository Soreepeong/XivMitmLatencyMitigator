...
def use_custom_opcodes():
	cfg = configparser.ConfigParser()
	cfg.read(SCRIPT_DIRECTORY + '/config.ini')

	use_opcodes = cfg.getboolean('Opcodes','use_custom_opcodes')

	return use_opcodes

def get_custom_opcodes():
    cfg = configparser.ConfigParser()
    cfg.read(SCRIPT_DIRECTORY + '/config.ini')
    C2S_ActionRequest = cfg.get('Opcodes','C2S_ActionRequest')
    C2S_ActionRequestGroundTargeted = cfg.get('Opcodes','C2S_ActionRequestGroundTargeted')
    S2C_ActionEffect01 = cfg.get('Opcodes','S2C_ActionEffect01')
    S2C_ActionEffect08 = cfg.get('Opcodes','S2C_ActionEffect08')
    S2C_ActionEffect16 = cfg.get('Opcodes','S2C_ActionEffect16')
    S2C_ActionEffect24 = cfg.get('Opcodes','S2C_ActionEffect24')
    S2C_ActionEffect32 = cfg.get('Opcodes','S2C_ActionEffect32')
    S2C_ActorCast = cfg.get('Opcodes','S2C_ActorCast')
    S2C_ActorControl = cfg.get('Opcodes','S2C_ActorControl')
    S2C_ActorControlSelf = cfg.get('Opcodes','S2C_ActorControlSelf')
...
def opcodes_from_config(definitions_fl):
    cust_opcodes_dict = get_custom_opcodes()
    definitions_fl[1].update(
    C2S_ActionRequest = cust_opcodes_dict["C2S_ActionRequest"],
    C2S_ActionRequestGroundTargeted = cust_opcodes_dict["C2S_ActionRequestGroundTargeted"],
    S2C_ActionEffect01 = cust_opcodes_dict["S2C_ActionEffect01"],
    S2C_ActionEffect08 = cust_opcodes_dict["S2C_ActionEffect08"],
    S2C_ActionEffect16 = cust_opcodes_dict["S2C_ActionEffect16"],
    S2C_ActionEffect24 = cust_opcodes_dict["S2C_ActionEffect24"],
    S2C_ActionEffect32 = cust_opcodes_dict["S2C_ActionEffect32"],
    S2C_ActorCast = cust_opcodes_dict["S2C_ActorCast"],
    S2C_ActorControl = cust_opcodes_dict["S2C_ActorControl"],
    S2C_ActorControlSelf = cust_opcodes_dict["S2C_ActorControlSelf"]
    )
    return definitions_fl
...
def load_definitions(update_opcodes: bool):
    ...
    definitions_raw = []
    if os.path.exists(definitions_filepath) and use_config_opcodes:
        with open(definitions_filepath, "r") as fp:
            data = json.load(fp)
            definitions_raw.append(data)
            logging.info("Replacing opcodes values with config.ini values")
            definitions_custom = opcodes_from_config(definitions_raw[0])
            return [OpcodeDefinition.from_dict(x) for x in definitions_custom]
    elif os.path.exists(definitions_filepath) and not use_config_opcodes:
        try:
            if update_opcodes or not use_config_opcodes:
                logging.info("Using default opcodes - Force API opcode definition")
                raise RuntimeError("Force update needed")
    ...
    try:
        ...
        for f in filelist:
            ...
            data["Name"] = f["name"]
            definitions_raw.append(data)
        if use_config_opcodes:
            definitions_fnl = opcodes_from_config(definitions_raw)
            logging.info("Using opcodes from config.ini on fresh file")
        else:
            definitions_fnl = definitions_raw
            logging.info("Using default opcodes from fresh file")
    except Exception as e:
        raise RuntimeError(f"Failed to load opcode definition") from e
    with open(definitions_filepath, "w") as fp:
        json.dump(definitions_fnl, fp)
    definitions = [OpcodeDefinition.from_dict(x) for x in definitions_fnl]
    return definitions 
