    ...
    *(void**)&virt[off_OodleFree] = (void*)&my_free;
    ...
    while (true) {
        ...
        if (hdr.TargetLength == 0xFFFFFFFFU) {
            dst.resize(src.size());
            dst.resize(((OodleNetwork1UDP_Encode*)&virt[off_OodleNetwork1UDP_Encode])(&state[0], &shared[0], &src[0], src.size(), &dst[0]));
            // std::cerr << "Encoded: res=0x" << dst.size() << std::endl;
        } else {
            dst.resize(hdr.TargetLength);
            if (!((OodleNetwork1UDP_Decode*)&virt[off_OodleNetwork1UDP_Decode])(&state[0], &shared[0], &src[0], src.size(), &dst[0], dst.size())) {
                dst.resize(0);
                dst.resize(hdr.TargetLength);
            }
        }
