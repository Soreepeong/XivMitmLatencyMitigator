                ...
                if self._xivalex.definitions[0].is_action_effect(int(ipc.subtype)):
                    ...
                    if effect.source_sequence == 0:
                        ...
                        while self.pending_actions and self.pending_actions[0].sequence != effect.source_sequence:
                            ...
                            logging.info(f"[{self._conn_id:>4}] \t┎ ActionRequest ignored for processing: "
                                         f"actionId={item.action_id:04x} sequence={item.sequence:04x}")
                    ...
                    if math.isclose(wait_time, original_wait_time):
                        logging.info(f"[{self._conn_id:>4}] S2C_ActionEffect: actionId={effect.action_id:04x} "
                    ...
                    if control.category == XivMessageIpcActorControlCategory.Rollback:
                        ...
                        logging.info(f"[{self._conn_id:>4}] S2C_ActorControlSelf/ActionRejected: "
                    ...
                    if control.category == XivMessageIpcActorControlCategory.CancelCast:
                        ...
                        if self.pending_actions:
                            self.pending_actions.popleft()
                        logging.info(f"[{self._conn_id:>4}] S2C_ActorControl/CancelCast: actionId={action_id:04x}")
                    ...
                    logging.info(f"[{self._conn_id:>4}] S2C_ActorCast: actionId={cast.action_id:04x} "
                                 f"type={cast.skill_type:04x} action_id_2={cast.action_id_2:04x} "
                                 f"time={cast.cast_time:.3f} target_id={cast.target_id:08x}")

            except Exception as e:
                logging.exception(f"[{self._conn_id:>4}] unknown error {e} occurred in upstream handler; skipping")
